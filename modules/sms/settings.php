...
final class HMN_CRM_SMS_Settings {
	...
	private function render_form( $tab, $s ) {
		?><form method="post" action="options.php"><?php settings_fields( 'hmn_crm_sms_settings_group' ); ?><table class="form-table"><?php if ( 'connection' === $tab ) : ?><tr><th><label for="melipayamak_api_key">کلید API کنسول</label></th><td><input type="password" class="regular-text" id="melipayamak_api_key" name="hmn_crm_sms_settings[melipayamak_api_key]" value="<?php echo esc_attr( $this->v( $s, 'melipayamak_api_key' ) ); ?>" autocomplete="new-password" /><p class="description">کلید API بخش توسعه‌دهندگان پنل ملی‌پیامک.</p></td></tr><tr><th><label for="melipayamak_sender">شماره فرستنده</label></th><td><input class="regular-text" id="melipayamak_sender" name="hmn_crm_sms_settings[melipayamak_sender]" value="<?php echo esc_attr( $this->v( $s, 'melipayamak_sender' ) ); ?>" placeholder="5000xxx" /><p class="description">یکی از خطوط تأییدشده در پنل ملی‌پیامک؛ برای ارسال متن آزاد با API Advanced استفاده می‌شود.</p></td></tr><tr><th><label for="hmn-crm-custom-css">Custom CSS</label></th><td><textarea class="large-text code" rows="8" id="hmn-crm-custom-css" name="hmn_crm_sms_settings[custom_css]"><?php echo esc_textarea( $this->v( $s, 'custom_css' ) ); ?></textarea></td></tr><?php else : $fields = array( 'melipayamak_otp_body_id' => 'کد تأیید OTP', 'melipayamak_booking_body_id' => 'ثبت نوبت جدید', 'melipayamak_booking_edit_body_id' => 'ویرایش نوبت', 'melipayamak_booking_cancel_body_id' => 'لغو نوبت', 'melipayamak_booking_reminder_body_id' => 'یادآوری نوبت' ); foreach ( $fields as $key => $label ) : ?><tr><th><label for="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></label></th><td><input type="number" min="0" class="small-text" id="<?php echo esc_attr( $key ); ?>" name="hmn_crm_sms_settings[<?php echo esc_attr( $key ); ?>]" value="<?php echo esc_attr( $this->v( $s, $key ) ); ?>" /></td></tr><?php endforeach; endif; ?></table><?php submit_button( 'ذخیره تنظیمات' ); ?></form><?php
	}

	/** Render OTP test form and result. */
	private function render_test( $s ) {
		$status = isset( $_GET['hmn_sms_test'] ) ? sanitize_key( wp_unslash( $_GET['hmn_sms_test'] ) ) : ''; if ( 'success' === $status ) { echo '<div class="notice notice-success"><p>پیامک با موفقیت به ملی‌پیامک ارسال شد.</p></div>'; } elseif ( 'error' === $status ) { echo '<div class="notice notice-error"><p>' . esc_html( isset( $_GET['message'] ) ? sanitize_text_field( wp_unslash( $_GET['message'] ) ) : 'ارسال پیامک ناموفق بود.' ) . '</p></div>'; }
		?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><?php wp_nonce_field( 'hmn_crm_sms_test' ); ?><input type="hidden" name="action" value="hmn_crm_sms_test" /><input type="hidden" name="method" value="advanced" /><table class="form-table"><tr><th><label for="hmn-test-phone">شماره موبایل</label></th><td><input required class="regular-text" id="hmn-test-phone" name="phone" placeholder="09123456789" /></td></tr><tr><th><label for="hmn-test-text">متن پیامک</label></th><td><textarea required class="large-text" rows="3" id="hmn-test-text" name="text">پیام تست از سامانه نوبت‌دهی</textarea><p class="description">با API Advanced و شماره فرستنده ذخیره‌شده در تنظیمات ارسال می‌شود.</p></td></tr></table><?php submit_button( 'تست ارسال پیامک' ); ?></form><?php
	}
	...
	public function handle_test() {
		...
		$text = isset( $_POST['text'] ) && is_scalar( $_POST['text'] ) ? sanitize_textarea_field( wp_unslash( $_POST['text'] ) ) : '';
		$method = isset( $_POST['method'] ) && is_scalar( $_POST['method'] ) ? sanitize_key( wp_unslash( $_POST['method'] ) ) : 'advanced';
		$result = 'otp' === $method ? ( new HMN_CRM_SMS() )->send_otp( $phone ) : ( new HMN_CRM_SMS() )->send_advanced( $phone, $text );
		$args = array( 'page' => self::PAGE_SLUG, 'tab' => 'test' ); if ( is_wp_error( $result ) ) { $args['hmn_sms_test'] = 'error'; $args['message'] = sanitize_text_field( $result->get_error_message() ); error_log( 'HMN CRM SMS test failed: ' . $args['message'] ); } else { $args['hmn_sms_test'] = 'success'; } wp_safe_redirect( add_query_arg( $args, admin_url( 'admin.php' ) ) ); exit;
	}
